...
function customerWhatsApp(phone, orderNumber) {
  ...
  return `https://wa.me/${normalized}?text=${encodeURIComponent(
    `Hello ${phone ? '' : 'there'}JA fashions. I am contacting you about order ${orderNumber}.`,
  )}`;
}
...
export default function AdminOrderDetails() {
  ...
  useEffect(() => {
    load().catch((err) => setError(err.response?.data?.message || 'Could not load order'));
  }, [id]);
  ...
  const update = async (patch) => {
    ...
    setError('');
    try {
      const res = await api.put(`/admin/orders/${id}`, patch);
      setOrder(res.data.order);
    } catch (err) {
  ...
  const customerLink = useMemo(
    () => (order ? customerWhatsApp(order.customerPhone, order.orderNumber) : '#'),
    [order],
  );
    ...
    return (
      <main className="p-5 lg:p-10">
        ...
        <div className="mt-8 rounded-[2rem] bg-white p-8 shadow-sm">
          <p className="text-stone-600">{error || 'Loading order…'}</p>
          {error && (
            <button
              type="button"
              onClick={() =>
              ...
              className="mt-4 inline-flex items-center gap-2 rounded-full bg-stone-950 px-4 py-2 text-sm font-semibold text-white"
            >
  ...
  return (
    <main className="p-5 lg:p-10">
      ...
      <div className="mt-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-amber-700">Order details</p>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center justify-center gap-2 rounded-full bg-emerald-600 px-5 py-3 font-semibold text-white"
        >
          Message customer on WhatsApp <ExternalLink size={16} />
        </a>
      </div>
        ...
        <section className="rounded-[2rem] bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-4">
            ...
            <div className="rounded-2xl bg-stone-950 p-5 text-white">
              ...
              <p className="mt-1 font-display text-4xl">{formatNaira(order.total)}</p>
      ...
      <section className="mt-5 rounded-[2rem] bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 grid gap-3">
          {(order.items || []).map((item) => (
            <div key={item.id} className="flex gap-4 rounded-2xl bg-stone-50 p-3">
              <div className="h-16 w-14 overflow-hidden rounded-xl bg-stone-200">
                {item.productImage && (
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-stone-500">
                  Qty {item.quantity} · {formatNaira(item.productPrice)}
                </p>
              </div>
        ...
        <div className="mt-5 flex items-center justify-between border-t border-stone-200 pt-4 font-semibold">
      ...
      {order.status === 'DELIVERED' && (
        <div className="mt-5 flex items-center gap-2 rounded-2xl bg-emerald-50 p-4 text-emerald-700">
          <CheckCircle2 size={18} /> This order is marked delivered.
        </div>
      )}
    </main>
  );
