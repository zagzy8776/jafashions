...
function customerWhatsApp(phone, orderNumber) {
  const digits = String(phone || '').replace(/\D/g, '');
  const normalized = digits.startsWith('0') ? `234${digits.slice(1)}` : digits;
  return `https://wa.me/${normalized}?text=${encodeURIComponent(`Hello ${phone ? '' : 'there'}JA fashions. I am contacting you about order ${orderNumber}.`)}`;
}
...
export default function AdminOrderDetails() {
  ...
  const load = async () => { const res = await api.get(`/admin/orders/${id}`); setOrder(res.data.order); };
  useEffect(() => { load().catch((err) => setError(err.response?.data?.message || 'Could not load order')); }, [id]);

  const update = async (patch) => {
    setSaving(true); setError('');
    try { const res = await api.put(`/admin/orders/${id}`, patch); setOrder(res.data.order); }
    catch (err) { setError(err.response?.data?.message || 'Could not update order'); }
    finally { setSaving(false); }
  };

  const customerLink = useMemo(() => order ? customerWhatsApp(order.customerPhone, order.orderNumber) : '#', [order]);

  if (!order) return <main className="p-5 lg:p-10"><Link to="/admin/orders" className="inline-flex items-center gap-2 text-sm text-stone-600"><ArrowLeft size={16}/> Back to orders</Link><div className="mt-8 rounded-[2rem] bg-white p-8 shadow-sm"><p className="text-stone-600">{error || 'Loading order…'}</p>{error && <button type="button" onClick={() => load().catch((err) => setError(err.response?.data?.message || 'Could not load order'))} className="mt-4 inline-flex items-center gap-2 rounded-full bg-stone-950 px-4 py-2 text-sm font-semibold text-white"><RefreshCw size={15}/> Try again</button>}</div></main>;

  return <main className="p-5 lg:p-10">
