    ...
    <main className="luxury-gradient grid min-h-screen place-items-center px-4 py-10">
      <form onSubmit={submit} className="w-full max-w-md rounded-[2.5rem] border border-amber-200/30 bg-[#fffaf1] p-8 shadow-2xl">
        ...
        <div className="mt-6 grid gap-4">
          <label className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm"><Mail size={18} className="text-amber-700" /><input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-transparent outline-none" placeholder="Admin email" /></label>
          <label className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-sm"><Lock size={18} className="text-amber-700" /><input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-transparent outline-none" placeholder="Password" /></label>
